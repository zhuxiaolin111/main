package com.jingang.app.tabhost;
...
import com.jingang.app.activity.BaseActivity;
import com.jingang.app.application.Constant;
import com.jingang.app.entity.BadgeView;
...
public class TabFragment extends Fragment implements
	...
	public int getTabLayoutHeight() {
		...
		if (height == 0) {
			height = (int) getResources().getDimension(
					R.dimen.tab_layout_default_height);
		}
			...
			for (int i = 0; i < size; i++) {
				...
				if (btnView instanceof ViewStub) {
					...
					if (!tabMode.isSetMidButton()) {
						...
						final Button middleButton = (Button) getView()
								.findViewById(R.id.tab_middle_button);
						...
						middleButton.setOnClickListener(new OnClickListener() {
							...
							@Override
							public void onClick(View v) {
								final TabMode mode = (TabMode) v
										.getTag(R.id.tab_mode_obj);
								if (mode == null) {
									return;
								}
								final int index = (Integer) radioButton
										.getTag(R.id.tab_index);
								if (mListener != null) {
									mListener.OnFocusChange(mode.getTabId(),
											index);
								}
							}
						});
	...
	public void showNewMsg(int tabId, boolean isShow) {
		try {
			...
			if (view instanceof RadioButton) {
				...
				if (isShow) {
					if (!badge.isShown()) {
						badge.setCompoundDrawablesWithIntrinsicBounds(0,
								R.drawable.new_tweet_icon, 0, 0);
			...
			if (view instanceof RadioButton) {
				...
				if (badge == null) {
					...
					final int color = getResources().getColor(
							R.color.new_msg_num_text_color);
				...
				if (msgNum > 0) {
					...
					if (!badge.isShown()) {
						badge.show(true);
						if (getActivity() != null) {
							final DisplayMetrics metric = getActivity()
									.getApplicationContext().getResources()
									.getDisplayMetrics();
							if (metric != null) {
								if (textSize > 8) {
									badge.setapplyLayoutParams(
											(int) getResources().getDimension(
													R.dimen.tab_un_read_margin),
											0, 0, 0);
								} else {
									if (metric.density >= 2) {
										badge.setapplyLayoutParams(
	...
	public void setTabExtensionView(View extensionView) {
		if (getView() != null) {
			final RelativeLayout tabExtensionLayout = (RelativeLayout) getView()
					.findViewById(R.id.tab_extension_layout);
	...
	public class TabsAdapter extends FragmentStatePagerAdapter implements
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			try {
				...
				// LogUtils.e("CustomFragmentStatePagerAdapter",
	...
	// private class DepthPageTransformer implements
	// CustomViewPager.PageTransformer {
	...
	// }
	//
	// private class ZoomOutPageTransformer implements
	// CustomViewPager.PageTransformer {
	 ...
	 */
	public void setTabOnDoubleClickListener(
			ITabOnDoubleClickListener onDoubleClickListener) {
